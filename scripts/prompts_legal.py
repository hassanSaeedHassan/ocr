...
CONTRACT_F_PROMPT2= """
    "You should only return sections found in the image."
    " Ignore those sections Detail Commission Broker Seller ,  Detail Commission Broker Buyer,Address information,Additional Information,Facilities, Tenancy Information ,Property Financial Information, Notes don't ever return those secitons data in the output.
    "Return a valid JSON object with no extra explanation or commentary. "
    "the (Information ID National) section could found twice with difference details data for different persons get both of them"
    "If this page belongs to the Terms and Conditions section, extract the following fields exactly:\n"
    "you will find a Table under Term number 6 containing the Payment details (payment type,amount,cheque number ,cheque date,bank name)"
    "get the data from this table under the key payment details"
    
    "ignore any other terms and conditions"
    "If the page does not belong to the Terms and Conditions section, you should return the data of each section in json format only the english data. "
     ...
     "If this page contain the seller broker details section or buyer broker details section, extract the following fields exactly:\n"
        ...
        "  - \" Email\"\n"
    "remember to fetch all the data except for the terms and condition and note the data is listed in two columns left and right "
    "for both National ID Information and Passport Information sections ensure you put the section name with counter  its occurance as a key for the json for example Property Details is the section name put it as key in json like this (property_details_1)"
    "Note National ID Information and Passport Information are two different sections"
    "don't put this ```json  in the response"
   """
