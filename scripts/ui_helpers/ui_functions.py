...
def render_person_roles_editor_old(results, key='person_roles_editor'):
    """
    ...
    df['Indvidual/Company']='Individual'
    df['Email']=None
    df['Phone']=None
    df.index += 1
    edited = st.data_editor(
        ...
        column_config={
           ...
           'Indvidual/Company': st.column_config.SelectboxColumn(
                'Indvidual/Company',
                options=[
                    'Individual', 'Company'
                ]
...
def render_person_roles_editor(results, key='person_roles_editor'):
    ...
    for cl in clusters:
        ...
        if 'poa' in cl['docs']:
            parts.append('POA')
        rows.append({'Name': cl['label'],
                     'Provided Documents': ', '.join(parts) or 'none',
                     'Role': cl['role']})

    # 7) Create DataFrame, ensuring columns exist even if rows is empty
    columns = ['Name', 'Provided Documents', 'Role', 'Indvidual/Company', 'Email', 'Phone']
    df = pd.DataFrame(rows, columns=columns)
    # add default columns
    if df.empty:
        # create an empty row so data_editor shows columns
        df = pd.DataFrame([{col: None for col in columns}])
    # set defaults
    df['Indvidual/Company'] = df['Indvidual/Company'].fillna('Individual')
